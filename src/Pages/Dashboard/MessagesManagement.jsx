...
const MessagesManagement = () => {
  const { role } = useRole();
  const axiosMessagesManagement = useAxiosSecure();
  ...
  // 🔥 fetch comments
  const fetchMessages = async (search = "", page = 1) => {
    try {
      setLoading(true);

      const res = await axiosMessagesManagement.get(
        `/messages/user?search=${search}&page=${page}&limit=${limit}`,
      );
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      fetchMessages(searchText, currentPage);
    }, 400);
  ...
  const handleView = async (id) => {
    try {
      const res = await axiosMessagesManagement.get(`/messages/${id}`); // ✅ FIX
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const res = await axiosMessagesManagement.delete(`/messages/${id}`);
    ...
    <div className="p-5 text-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 p-4 rounded-2xl bg-white/5 backdrop-blur-xl border border-cyan-400/20 shadow-lg">
        ...
        <h1 className="text-3xl font-bold text-cyan-300 tracking-wide flex items-center gap-1">
          <Users size={32} />
          <span>
            Messages <span className="text-xs">({totalMessages})</span>{" "}
          </span>
        </h1>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-6">No Message found</div>
      ) : (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="avatar">
                        <div className="h-12 w-12 rounded-xl ring ring-cyan-400/30 ring-offset-2 ring-offset-cyan-900/20">
                          <img src={user.senderImage} alt="User" />
                        </div>
                      </div>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      <div className="avatar">
                        <div className="h-12 w-12 rounded-xl ring ring-cyan-400/30 ring-offset-2 ring-offset-cyan-900/20">
                          <img src={user.receiverImage} alt="User" />
                        </div>
                      </div>
        ...
        <div className="modal-box bg-gradient-to-br from-cyan-900/40 to-black/40 backdrop-blur-lg border border-cyan-400/30 shadow-xl rounded-2xl text-white">
          ...
          <div className="flex justify-between items-center">
            <div className="flex flex-col items-center text-center flex-1">
              <img
                src={selectedMessages?.senderImage}
                alt="user"
                className="w-24 h-24 rounded-full border-4 border-cyan-400 shadow-lg mb-3"
              />

              <h3 className="text-xl font-bold text-cyan-300">
                {selectedMessages?.senderName}
              </h3>

              <p className="text-sm text-gray-300">
                {selectedMessages?.senderEmail}
              </p>
            </div>
            <div className="flex flex-col items-center text-center flex-1">
              <img
                src={selectedMessages?.receiverImage}
                alt="user"
                className="w-24 h-24 rounded-full border-4 border-cyan-400 shadow-lg mb-3"
              />

              <h3 className="text-xl font-bold text-cyan-300">
                {selectedMessages?.receiverName}
              </h3>

              <p className="text-sm text-gray-300">
                {selectedMessages?.receiverEmail}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-3 text-sm">
            <div className="flex items-center gap-2">
              <span className="text-gray-400 whitespace-nowrap">Message</span>
              <div className="flex-1 border-b border-dashed border-cyan-400/30"></div>
              <span className="text-right">{selectedMessages?.text}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 border-b border-dashed border-cyan-400/30"></div>
              <span className="text-right">
                {new Date(selectedMessages?.updatedAt).toLocaleString()}
              </span>
            </div>
